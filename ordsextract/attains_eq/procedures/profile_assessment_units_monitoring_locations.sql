CREATE OR REPLACE PROCEDURE attains_eq.profile_assessment_units_monitoring_locations(
...
BEGIN
   ...
   IF NOT boo_mute
   THEN
      HTP.PRN('{"name":"profile_assessment_units_monitoring_locations","records":[');
   ...
   FOR json_rec IN ( 
      SELECT /*+ INDEX(PROFILE_ASSESSMENT_UNITS_MONITORING_LOCATIONS_UX1) NO_PARALLEL(a) */ 
      JSON_OBJECT(
         ...
         ,KEY 'monitoringlocationorgid'     VALUE a.monitoringlocationorgid
         ,KEY 'monitoringlocationid'        VALUE a.monitoringlocationid
         ,KEY 'monitoringlocationdatalink'  VALUE a.monitoringlocationdatalink
      ...
      FROM
      attains_app.profile_assessment_units_monitoring_locations a
      WHERE
          a.row_id >  int_offset
...
END profile_assessment_units_monitoring_locations;
/

GRANT EXECUTE ON attains_eq.profile_assessment_units_monitoring_locations TO attains_eq_rest;
